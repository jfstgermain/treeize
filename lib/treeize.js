...
var isEmpty = function(item) {
  return item == null ||
    (_.isArray(item) && _.isEmpty(item)) ||
    (_.isPlainObject(item) && (_.isEmpty(item) || _.every(_.values(item), _.isNil)));
}
...
Treeize.prototype.signature = function(row, options, auto) {
  ...
  var isRowAnArray  = _.isArray(row)
  var opt           = _.merge({}, this._options, options || {})

  this.data.signature.type = isRowAnArray ? 'array' : 'object'
...
Treeize.prototype.grow = function(data, options) {
  ...
  if (typeCheck(data) !== 'array' || !data.length) {
    return this
  }
        ...
        // NORMAL NODE TRAVERSAL
          ...
          if (node.isCollection) {
            ...
            if (!trail[node.name]) {
              ...
              if (!isEmpty(blueprintExtended)) {
                trail[node.name].push(blueprintExtended);
            ...
            if (!isEmpty(blueprintExtended)) {
              if (!trail[node.name] || (opt.output.objectOverwrite && (typeCheck(trail[node.name]) !== typeCheck(blueprintExtended)))) {
                ...
                if (typeCheck(trail[node.name]) === 'object') {
                  // console.log({ trails, trail, node }, '****************************** FUCK');
                  trail[node.name] = _.merge({}, trail[node.name], blueprintExtended)
                }
...
Treeize.prototype.resetOptions = function() {
  this._options = _.merge({}, this.baseOptions)
